...
const CHAPTER_THEMES = {
  ...
  1: {
    ...
    cageHighlight: 'rgba(34,197,94,0.22)',
  ...
  2: {
    ...
    cageHighlight: 'rgba(99,102,241,0.22)',
  ...
  3: {
    ...
    cageHighlight: 'rgba(139,92,246,0.22)',
  ...
  4: {
    ...
    cageHighlight: 'rgba(217,119,6,0.22)',
  ...
  5: {
    ...
    cageHighlight: 'rgba(168,85,247,0.22)',
  ...
  6: {
    ...
    cageHighlight: 'rgba(251,191,36,0.22)',
...
const CHAPTER_BG_IMAGES = {
  1: 'assets/images/bg/bg_chapter_1.webp',
  2: 'assets/images/bg/bg_chapter_2.webp',
  3: 'assets/images/bg/bg_chapter_3.webp',
  4: 'assets/images/bg/bg_chapter_4.webp',
  5: 'assets/images/bg/bg_chapter_5.webp',
  6: 'assets/images/bg/bg_chapter_6.webp',
  7: 'assets/images/bg/bg_chapter_7.webp',
};
...
class Renderer {
  ...
  _drawBoardCache(board, canvasSize) {
    ...
    if (board.cages && board.cages.length > 0) {
      ...
      board.cages.forEach(cage => {
        ...
        // 和值文字（放在笼子上边框上方，不遮挡格子内容）
        let minR = size, minC = size;
        cage.cells.forEach(([r, c]) => {
          if (r < minR) { minR = r; minC = c; }
          else if (r === minR && c < minC) { minC = c; }
        });
        const tx = minC * cellSize + 2;
        // 第0行放在格子内顶部，其他行放在上边框上方
        const ty = minR === 0 ? (minR * cellSize + sumFontSize + 1) : (minR * cellSize - 1);
        ctx.setLineDash([]);
        // 文字描边确保清晰
        ctx.strokeStyle = theme.bgColor || '#ffffff';
        ctx.lineWidth = 3;
        ctx.strokeText(sumText, tx, ty);
        // 文字主体
        ctx.fillStyle = theme.cageSumText || theme.cageBorder || theme.fixedNum;
        ctx.fillText(sumText, tx, ty);
        ctx.setLineDash([4, 3]);
      });
  ...
  _applyPageBg(chapterId) {
    ...
    if (bgSrc) {
      ...
      testImg.onerror = function() {
        document.body.style.backgroundImage = `url('${bgFallback}')`;
      };
      testImg.src = bgSrc;
  ...
  _drawCages(board) {
    ...
    for (let depth = 0; depth <= maxDepth; depth++) {
      ...
      const lineWidth = isInner ? 1.5 : 2.5;
      const dashStyle = isInner ? [] : [5, 3];
      const strokeColor = isInner ? theme.accent : theme.cageDash;
      ...
      layerCages.forEach(cage => {
        ...
        // 绘制和值文字（参考Logic Wiz风格：放在笼子上边框上方，不遮挡格子内容）
        if (isSumVisible(minR, minC)) {
          ctx.setLineDash([]);
          const sumFontSize = Math.max(9, Math.floor(cellSize * 0.20));
          ctx.font = `700 ${sumFontSize}px sans-serif`;
          ctx.textAlign = 'left';
          ctx.textBaseline = 'bottom';
          ...
          let textX, textY;
          if (minR === 0) {
            // 第0行笼子：放在格子内部左上角（上面没有空间）
            textX = minC * cellSize + 2;
            textY = minR * cellSize + sumFontSize + 1 + offsetY;
          } else {
            // 非第0行：放在笼子上边框的上方（外部），不占用格子空间
            textX = minC * cellSize + 2;
            textY = minR * cellSize - 1 + offsetY;
          }

          // 文字描边：用浅色背景描边确保在任何背景下都清晰
          ctx.strokeStyle = theme.bgColor || '#ffffff';
          ctx.lineWidth = 3;
          ctx.strokeText(sumText, textX, textY);

          // 文字主体
          ctx.fillStyle = badgeText || theme.cageDash;
  ...
  _drawCandidates(board) {
    ...
    // 笔记区域：左上角留出空间给笼子和值标签（参考Logic Wiz风格）
    const paddingTop = Math.max(4, cellSize * 0.15);  // 顶部留白
    const paddingLeft = Math.max(4, cellSize * 0.08); // 左侧留白
    ...
    const fontSize = Math.max(8, Math.floor(cellSize * 0.17));
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.font = fontSize + 'px sans-serif';

    // 如果有笔记系统，使用它来控制每格的显示状态
    const noteSys = window.gameNoteSystem || null;
    ...
    for (let r = 0; r < size; r++) {
      for (let c = 0; c < size; c++) {
        ...
        if (noteSys) {
          ...
          // 兼容旧版：笔记模式下全显
          show = board.inputMode === 'candidate';
        }
