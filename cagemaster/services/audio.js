...
const AudioManager = {
  ...
  playDoorOpen() {
    ...
    // 1. 门轴吱呀声主音（锯齿波 + 带通滤波，2kHz左右，人耳最敏感区域）
  ...
  playLaugh() {
    ...
    // 连续3-4个上升的"呵"音
    const laughs = 3 + Math.floor(Math.random() * 2);
  ...
  playPaperFlip() {
    ...
    // 1. 翻纸的沙沙声（带通白噪声）
    const paperLen = this.ctx.sampleRate * 0.25;
  ...
  playBookFlip(pages = 5) {
    ...
    // 生成连续翻页的噪声
    const bookLen = this.ctx.sampleRate * totalDur;
  ...
  playAmbientWind(intensity = 0.3) {
    if (!this.enabled || !this.ctx || !this.sfxEnabled) return;
    this.resume();
    if (this._windNode) return; // 已经在播放
