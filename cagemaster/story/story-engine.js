...
const StoryEngine = (function() {
  ...
  // ---- 立绘路径辅助：优先 WebP，失败回退 PNG ----
  function _portraitSrc(file) {
    return `assets/images/portraits/${file.replace(/\.(png|jpg|webp)$/, '')}.webp`;
  }
  function _portraitFallbackSrc(file) {
    return `assets/images/portraits/${file.replace(/\.(png|jpg|webp)$/, '')}.png`;
  }
  // 用 Image 对象尝试加载 WebP，失败自动切 PNG
  function _loadPortrait(file, onLoad) {
    const webpSrc = _portraitSrc(file);
    const pngSrc = _portraitFallbackSrc(file);
    const img = new Image();
    let done = false;
    img.onload = function() {
      if (done) return;
      done = true;
      onLoad && onLoad(webpSrc);
    };
    img.onerror = function() {
      ...
      pngImg.onload = function() { onLoad && onLoad(pngSrc); };
      pngImg.onerror = function() { onLoad && onLoad(pngSrc); };
      pngImg.src = pngSrc;
    };
    img.src = webpSrc;
  }
  ...
  function playVoice(voiceId) {
    ...
    const mp3Path = `assets/audio/voices/${voiceId}.mp3`;
    const wavPath = `assets/audio/voices/${voiceId}.wav`;
  ...
  function playDialogues(voiceIds, callback) {
    ...
    // 派发"台词开始"事件，用于暂停计时器等
    try {
      document.dispatchEvent(new CustomEvent('story-dialogue-start'));
    } catch(e) {}
    _playNext();
  }
  ...
  function _endScene() {
    ...
    // 派发"台词结束"事件，用于恢复计时器等
    try {
      document.dispatchEvent(new CustomEvent('story-dialogue-end'));
    } catch(e) {}
