...
class StoryModal {
  ...
  _build() {
    ...
    // WebP 加载失败回退：PNG → JPG → 隐藏（显示emoji）
    this.characterImg.addEventListener('error', function() {
      const img = this;
      if (!img._fb1) {
        img._fb1 = 1;
        ...
        img._fb2 = 1;
        img.src = img.src.replace(/\.png$/, '.jpg').replace(/\.webp$/, '.jpg');
      } else {
        img.style.display = 'none';
        const emoji = img.parentElement && img.parentElement.querySelector('.story-character-emoji');
  ...
  _showCurrent() {
    ...
    if (!dlg.avatar && !dlg.icon && speakerName) {
      ...
      else if (speakerName === '阿妍') preset = chars.ayan;
      else if (speakerName === '设局人') preset = chars.setter;
      else if (speakerName === '设局人残影' || speakerName === '设局人（残影）' || speakerName === '残影') preset = chars.shadow;
      else if (speakerName === '设局人秘术' || speakerName === '设局人（秘术）' || speakerName === '秘术形态') preset = chars.setterSecret;
      ...
      else if (speakerName === '你' || speakerName === '玩家') preset = null;
    }
...
/**
 * 便捷函数：用角色预设快速构建对话条目
 * @param {string} role - 'keeper'|'yan'|'setter'|'narrator'
