  ...
  _createUI() {
    ...
    if (avatarEl) {
      if (this.opponent.avatar && (this.opponent.avatar.endsWith('.png') || this.opponent.avatar.endsWith('.jpg') || this.opponent.avatar.endsWith('.webp'))) {
        avatarEl.innerHTML = `<img src="${this.opponent.avatar}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;" onerror="if(!this._fb1){this._fb1=1;this.src=this.src.replace(/\\.webp$/,'.png');}else if(!this._fb2){this._fb2=1;this.src=this.src.replace(/\\.png$/,'.jpg').replace(/\\.webp$/,'.jpg');}else{this.style.display='none'}">`;
      } else {
...
const BOSS_CONFIGS = {
  ...
  109: {
    name: '阿岩',
    avatar: 'assets/images/ayan-avatar.webp',
    color: '#22c55e',
    speedMin: 6000,
    speedMax: 11000,
    mistakeChance: 0.15,
  ...
  208: {
    name: '守笼人',
    avatar: 'assets/images/keeper-avatar.webp',
    color: '#6366f1',
    speedMin: 3500,
    speedMax: 6000,
    mistakeChance: 0.05,
  ...
  406: {
    name: '残局守护者',
    avatar: 'assets/images/guardian-avatar.webp',
    color: '#f97316',
    speedMin: 2000,
    speedMax: 3800,
    mistakeChance: 0.02,
  ...
  506: {
    name: '星辰梭',
    avatar: 'assets/images/starshuttle-avatar.webp',
    color: '#a855f7',
    speedMin: 1500,
    speedMax: 2800,
    mistakeChance: 0.01,
  ...
  606: {
    name: '设局人',
    avatar: 'assets/images/setter-avatar.webp',
    color: '#dc2626',
    speedMin: 1000,
    speedMax: 2000,
    mistakeChance: 0.005,
  ...
  706: {
    ...
    mistakeChance: 0.003,
    fillStyle: 'surround',
    personality: '秘术全开的设局人，运用二连纵横阵、三才游鱼阵等高级技巧，速度极快但仍保持大师的优雅',
    preDialog: [
      { speaker: '设局人（秘术）', text: '看来你已经读过了全部六卷秘术。' },
      { speaker: '阿岩', text: '他的气场变了……比终局时还要强！' },
      { speaker: '设局人（秘术）', text: '这一次，我不会留手。数对、三子法、二连纵横阵、三才游鱼阵——我会全部用出来。' },
    ...
    warningLines: [
      '设局人秘术全开了！二连纵横阵和三才游鱼阵接连出手，快追！',
      '他在使用三才游鱼阵！不能让他完成！'
    ],
    encounterLines: {
      far:  [
        { text: '设局人：看到那个数对了吗？我看到了。', intensity: 'light' },
        { text: '设局人：三子法……你还没发现吗？', intensity: 'light' },
        { text: '设局人：我用二连纵横阵删去了你的可能性。', intensity: 'light' }
      ],
      mid:  [
        { text: '设局人：三才游鱼阵已经布下，你逃不掉了。', intensity: 'medium' },
