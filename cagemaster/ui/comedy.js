...
const ComedySystem = {
  ...
  _charPortrait(speaker, emotion) {
    const emotions = {
      ...
      '阿妍':   { default: 'Yan_default', smile: 'Yan_smile', angry: 'Yan_angry', surprised: 'Yan_surprised', lose: 'Yan_lose' },
    };
    ...
    const img = file ? `assets/images/portraits/${file}.webp` : null;
    const imgFallback = file ? `assets/images/portraits/${file}.png` : null;
    return { img, imgFallback, emotion: emoKey };
  },
  ...
  _showBubble(speaker, text, color, emoji, emotion) {
    ...
    const avatarHtml = portrait.img
      ? `<img src="${portrait.img}" data-fallback="${portrait.imgFallback}" style="width:${isMobile ? '36px' : '42px'};height:${isMobile ? '36px' : '42px'};border-radius:50%;object-fit:cover;flex-shrink:0;border:2px solid rgba(255,255,255,0.3);box-shadow:0 2px 8px rgba(0,0,0,0.3);" onerror="if(!this.dataset.fallbackDone){this.dataset.fallbackDone='1';this.src=this.dataset.fallback;}else{this.style.display='none';this.nextElementSibling.style.display='inline';}">`
  ...
  ayanSays(key) {
    ...
    if (typeof window._ayanSay === 'function') {
      ...
      window.StoryEngine.sayAmbient('yan', 'smile', line);
    } else {
      this._showBubble('阿妍', line, 'linear-gradient(135deg,#22c55e,#15803d)', '🍃');
    }
  },
    ...
    if (typeof window.StoryEngine !== 'undefined' && window.StoryEngine) {
      ...
      const next = () => {
        ...
        const charMap = {
          ...
          '阿妍': { color: 'linear-gradient(135deg,#22c55e,#15803d)', emoji: '🍃' },
  ...
  // ---------- 事件：二连纵横阵出错 ----------
  onXwingWrong() {
    if (Math.random() < 0.3) this.keeperSays('二连纵横阵Wrong');
  },
