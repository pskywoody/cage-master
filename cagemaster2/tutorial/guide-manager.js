  ...
  _fireTrigger(trigger, eventData) {
    ...
    const doFire = () => {
      switch (type) {
        ...
        case 'tutorial':
          // 交互式分步教学
          this._startTutorial(trigger, config, eventData);
          break;
  ...
  _startTutorial(trigger, config, eventData) {
    ...
    if (!window._tutorialRunner) {
      window._tutorialRunner = new TutorialRunner({
        ...
        canvas: this.canvas,
        onComplete: () => {
          console.log('[GuideManager] 交互式教学完成');
    ...
    // 启动教程
    window._tutorialRunner.start(tutorialConfig);
    this._currentTutorial = tutorialConfig.id;
  }
