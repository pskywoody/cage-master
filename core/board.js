...
class Board {
  constructor(size = 9) {
    ...
    this._onRender = null;     // 划选状态变更渲染回调（game.html 绑定 gameApp.render）
    this.history = [];
    ...
    this.settings = {
      ...
      autoClearCandidates: true,  // 填数自动清除关联笔记（2026-08-04：笔记添加仍手动，消除自动化——填数后同行/列/宫/笼的该数字笔记自动划掉）
      muteAll: false,        // 一键静音
      bgm: true,             // 背景音乐
      sfx: true,             // 音效
    ...
    // Loop② 推导响应：某格有效笔记收敛到唯一候选时回调 (r, c, value) => void
    this.onNakedSingleReached = null;
    // Loop② 已收敛基线：Set<'r,c'>，首次 refreshDeductions 建立、不抛出
    this._convergedCells = null;
  ...
  loadLevel(puzzle) {
    ...
    // Loop②：新关卡重置推导基线（首次 refreshDeductions 仅建基线、不抛出）
    this._convergedCells = null;
    ...
    for (let r = 0; r < this.size; r++) {
      for (let c = 0; c < this.size; c++) {
        ...
        cell.cageId = null;
        cell.cageIds = [];
        // v2.0：重置 Boss 战残留标记（tpl 控制器写入，跨关复用 Board 时不残留）
  ...
  /**
   * V4.3.25（Spec v1.4）：获取指定格的笔记（候选数）数量
   * @param {number} r
   * @param {number} c
   * @returns {number} 笔记数量（0 = 无笔记/已清空）
   */
  getNotesCount(r, c) {
    const cell = this.cells[r]?.[c];
    if (!cell) return 0;
    if (cell.candidates instanceof Set) return cell.candidates.size;
    return Array.isArray(cell.candidates) ? cell.candidates.length : 0;
  }

  /**
   * V4.3.25（Spec v1.4）：获取指定格的笔记（候选数）数组
   * @param {number} r
   * @param {number} c
  ...
  getNotesArray(r, c) {
    ...
    return Array.isArray(cell.candidates) ? cell.candidates.slice() : [];
  }
  ...
  startPaintSelect(r, c) {
    ...
    this.paintedCells = new Set();
    this._paintLastCell = { r, c }; // V4.3.32：路径补全用的上一个格
    // 起始格加入选中
    this._addPaintedCell(r, c);
  }

  /**
   * 更新画笔选格（滑过的格子都加选，路径上的中间格也补齐）
   */
  updatePaintSelect(r, c) {
    if (!this.isPaintSelecting) return;
    if (r < 0 || r >= this.size || c < 0 || c >= this.size) return;
    // V4.3.32：从上一格到当前格做直线插值，快速拖动时中间格不漏选
    const from = this._paintLastCell || { r, c };
    const dr = r - from.r;
    const dc = c - from.c;
    const steps = Math.max(Math.abs(dr), Math.abs(dc));
    if (steps > 0) {
      for (let i = 1; i <= steps; i++) {
        const nr = Math.round(from.r + dr * i / steps);
        const nc = Math.round(from.c + dc * i / steps);
        if (nr < 0 || nr >= this.size || nc < 0 || nc >= this.size) continue;
        this._addPaintedCell(nr, nc);
      }
    } else {
      this._addPaintedCell(r, c);
    }
    this._paintLastCell = { r, c };
  }
  ...
  _addPaintedCell(r, c) {
    ...
    // v2.0：划选状态变更触发渲染回调（补 _dragDown 起点格与 _dragUp 终点态的渲染缺口）
  ...
  updateCandidates(options = {}) {
    ...
    // 后台内部：候选数变化（填/擦/撤销/重做）后刷新无效笔记知识（不渲染给玩家）
    this.updateInvalidNotes();
  }

  /**
   * 校验单个候选笔记是否与当前盘面冲突（后台内部判断，不渲染给玩家）
   * 复用 _getCellCandidates 的完整约束（行/列/宫/笼和），保证判定与引擎一致。
   * 有效返回 null；无效返回 { reason }，reason ∈ row|col|box|cage|range|self。
   * @param {number} r - 行
   * @param {number} c - 列
   * @param {number} num - 候选数字
   * @param {Array|null} [grid] - 可选，已构建的盘面矩阵（避免重复构建）
   * @returns {{reason:string}|null}
   */
  validateNote(r, c, num, grid) {
    try {
      const cell = this.cells && this.cells[r] && this.cells[r][c];
      if (!cell) return { reason: 'range' };
      if (num < 1 || num > this.size) return { reason: 'range' };
      if (cell.fixedNum === num || cell.fillNum === num) return { reason: 'self' };
      if (cell.fixedNum || cell.fillNum) return { reason: 'filled' };
      const g = grid || this._buildGrid();
      // 权威判定：引擎候选集是否仍包含该数
      if (this._getCellCandidates(g, r, c).indexOf(num) >= 0) return null;
      // 定位原因（面向玩家）
      for (let cc = 0; cc < this.size; cc++) if (cc !== c && g[r][cc] === num) return { reason: 'row' };
      for (let rr = 0; rr < this.size; rr++) if (rr !== r && g[rr][c] === num) return { reason: 'col' };
      const { boxW, boxH } = this.getBoxSize();
      const br = Math.floor(r / boxH) * boxH, bc = Math.floor(c / boxW) * boxW;
      for (let dr = 0; dr < boxH; dr++) for (let dc = 0; dc < boxW; dc++) {
        const rr = br + dr, cc = bc + dc;
        if ((rr !== r || cc !== c) && g[rr][cc] === num) return { reason: 'box' };
      }
      return { reason: 'cage' };
    } catch (e) {
      return null; // 出错时保守视为有效，不误报
    }
  }

  /**
   * 刷新全盘无效笔记（后台内部知识，不渲染给玩家）
   * 笔记对错只由后台掌握：玩家不可见任何标记，笔记保持私有工作区语义。
   * 结果写入各空格 cell._invalidNotes = Set<num>，供后台逻辑（提示/教学/AI 等）读取。
   * 与 Loop②（推导）无关——玩家是否据此填数由玩家自主决定。
   */
  updateInvalidNotes() {
    try {
      const grid = this._buildGrid();
      for (let r = 0; r < this.size; r++) {
        for (let c = 0; c < this.size; c++) {
          ...
          if (!cell.candidates || cell.candidates.size === 0) { cell._invalidNotes = new Set(); continue; }
          const invalid = new Set();
          for (const num of cell.candidates) {
            const res = this.validateNote(r, c, num, grid);
            if (res) invalid.add(num);
          }
          cell._invalidNotes = invalid;
        }
      }
    } catch (e) {
      // 失败静默，不破坏棋盘
  ...
  /**
   * 检测"有效笔记收敛到唯一候选"的空格（Loop ②：推导响应）
   *
   * 背景：玩家笔记是私有工作区，对错由后台静默掌握（_invalidNotes）。
   * 当 (cell.candidates - cell._invalidNotes).size === 1 时，该格目标已收敛，
   * 抛出 onNakedSingleReached(r, c, value)，供界面做轻量提示"目标已收敛"，
   * 等待玩家主动点击确认填入。
   *
   * 首次调用（_convergedCells 为 null）仅建立基线、不抛出，避免开局误报。
   * @returns {Array<{r:number,c:number,value:number}>} 本次新收敛的格子
   */
  refreshDeductions() {
    try {
      // 确保后台无效笔记知识最新（填/擦/笔记切换后候选或已变）
      this.updateInvalidNotes();
      const newly = [];
      const converged = new Set();
      const isFirst = this._convergedCells == null;
      if (isFirst) this._convergedCells = new Set();
      for (let r = 0; r < this.size; r++) {
        for (let c = 0; c < this.size; c++) {
          const cell = this.cells[r][c];
          if (!cell || cell.fixedNum || cell.fillNum) continue;
          if (!cell.candidates || cell.candidates.size === 0) continue;
          // 有效集合 = 玩家笔记 - 后台无效笔记；恰好剩 1 个才视为收敛
          let value = null;
          for (const num of cell.candidates) {
            if (cell._invalidNotes && cell._invalidNotes.has(num)) continue;
          ...
          const key = r + ',' + c;
          converged.add(key);
          if (isFirst || !this._convergedCells.has(key)) {
            newly.push({ r, c, value });
            if (typeof this.onNakedSingleReached === 'function') {
              try { this.onNakedSingleReached(r, c, value); } catch (e) {}
            }
          }
        }
      }
      this._convergedCells = converged;
      return newly;
    } catch (e) {
      return [];
    }
