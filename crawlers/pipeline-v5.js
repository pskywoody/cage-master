...
function classifyPuzzle(board, rating) {
  ...
  if (givens === 0) {
    ...
    if (score < 150) return 'pure-1star';
    if (score < 300) return 'pure-2star';
    if (score < 500) return 'pure-3star';
    if (score < 750) return 'pure-4star';
    return 'pure-5star';
  }
...
function main() {
  ...
  const outDir = path.join(__dirname, '..', 'game-src', 'data', 'puzzles', 'v5');
  
  console.log('=== Killer Sudoku Pipeline v5 ===\n');
  console.log('核心：最低可用技巧评级 + 卡壳点残局\n');
