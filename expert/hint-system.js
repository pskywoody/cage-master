/**
 * ============================================================
 *  HintSystem - 增强型提示系统（V4 ES Module版）
 * ============================================================
 *
 *  底层推理完全基于 TechRater 求解引擎。
 ...
 *
 *  移植自 cagemaster3/expert/hint-system.js
 *  转换：IIFE → ES Module，添加 try-catch 保护
 *
 * ============================================================
 */

// tech-rater.js 为 IIFE 全局脚本（<script src="core/tech-rater.js"> 加载后挂载 globalThis.TechRater）
// 与 battle-manager.js 一致：不静态 import，运行时从全局获取
const TechRater = (typeof globalThis.TechRater !== 'undefined') ? globalThis.TechRater : null;

// V4.3.26：本地化角色提示模板库（3 角色 × 技巧模板 + 证据动态填充）
...
export class HintSystem {
  constructor(board, solution, options = {}) {
    ...
    // 2026-08-03：首次提示锁定教学目标技巧
    // 当 preferredTechnique 设置后，每次 getHint() 都会优先尝试该技巧，
    // 直到首次成功展示目标技巧（支持"延迟结构"关卡：先前置填充，技巧随后浮现）
    this.preferredTechnique = options.preferredTechnique || null;
    this._targetTechShown = false;
  ...
  getHint() {
    ...
    // Q5 修复：点提示不再"自动填满笔记"——_findNextDeduction 内部会调
    // board.updateCandidates() 计算候选数（推理需要），但会污染玩家棋盘笔记。
    // 快照候选数，计算后恢复原样（不还原则玩家看到整盘笔记突然出现）。
    const savedCandidates = this._snapshotCandidates();
    let deduction = null;
    try {
      deduction = this._findNextDeduction();
    } finally {
    ...
    // ===== 证据归一化（微型教学 Loop 数据源，2026-08-03）=====
    // 补全标准字段：regionType / regionIndex / excludedNumbers
    // 源证据使用 scopeType/scopeIndex（区域）与 eliminated/eliminatedPositions（排除）
    const rawEvidence = deduction.evidence || null;
    let evidence = rawEvidence;
    if (rawEvidence && typeof rawEvidence === 'object') {
      const regionType = rawEvidence.scopeType ||
        (deduction.region && deduction.region.type) ||
        null;
      const regionIndex = (rawEvidence.scopeIndex !== undefined) ? rawEvidence.scopeIndex :
        (deduction.region && deduction.region.index !== undefined ? deduction.region.index : null);
      // excludedNumbers：优先取 eliminated（数字数组），其次从 eliminatedPositions / eliminatedCells 位置推导
      // Q14：eliminated 可能是对象数组（pointingClaiming/nakedTriplet 存 {row,col,num}）——
      // 此时从 num 提取数字，并把坐标并入 eliminatedPositions（否则排除红叉动画丢失）
      let excludedNumbers = [];
      const rawElim = rawEvidence.eliminated;
      const rawElimIsObjArr = Array.isArray(rawElim) && rawElim.length > 0 &&
        typeof rawElim[0] === 'object' && rawElim[0] !== null && rawElim[0].row !== undefined;
      if (Array.isArray(rawElim) && !rawElimIsObjArr) {
        excludedNumbers = rawElim.slice();
      } else if (rawElimIsObjArr) {
        excludedNumbers = rawElim.map(p => (p.num !== undefined ? p.num : p.value)).filter(v => typeof v === 'number');
      }
      if (excludedNumbers.length === 0 && Array.isArray(rawEvidence.eliminatedPositions)) {
        excludedNumbers = rawEvidence.eliminatedPositions.map(p => {
          if (p && typeof p === 'object') {
            const v = p.value !== undefined ? p.value : (p.num !== undefined ? p.num : null);
            return v;
          }
          return null;
        }).filter(v => typeof v === 'number');
      }
      if (excludedNumbers.length === 0 && Array.isArray(rawEvidence.eliminatedCells)) {
        excludedNumbers = rawEvidence.eliminatedCells.map(p => {
          if (p && typeof p === 'object') {
            const v = p.num !== undefined ? p.num : (p.value !== undefined ? p.value : null);
            return v;
          }
          return null;
        }).filter(v => typeof v === 'number');
      }
      // eliminatedPositions：优先取原字段，其次从 eliminatedCells / eliminated(对象数组) 归一化
      // （供 hint-adapter 红叉动画）
      let eliminatedPositions = Array.isArray(rawEvidence.eliminatedPositions)
        ? rawEvidence.eliminatedPositions.slice() : [];
      // Q14：hiddenSingle 的 eliminatedPositions 条目是 {cell:[r,c], reasons}——
      // 转成 {row,col,reasons}，否则渲染层红叉动画丢失，玩家看不到排除证据
      if (eliminatedPositions.length > 0) {
      ...
      if (eliminatedPositions.length === 0 && rawElimIsObjArr) {
        eliminatedPositions = rawElim
          .filter(p => p && typeof p.row === 'number' && typeof p.col === 'number')
          .map(p => ({ row: p.row, col: p.col, num: p.num }));
      }
      if (eliminatedPositions.length === 0 && Array.isArray(rawEvidence.eliminatedCells)) {
        eliminatedPositions = rawEvidence.eliminatedCells
          .filter(p => p && typeof p === 'object' && typeof p.row === 'number' && typeof p.col === 'number')
          .map(p => ({ row: p.row, col: p.col, value: (p.num !== undefined ? p.num : p.value) }));
      }
      evidence = Object.assign({}, rawEvidence, {
        regionType: regionType || null,
        regionIndex: (regionIndex === undefined) ? null : regionIndex,
    ...
    return {
      ...
      // 证据链（Level 3 可用，已归一化 regionType/regionIndex/excludedNumbers）
      evidence: evidence,
    ...
    catch (e) {      console.warn('HintSystem.getHint 异常:', e);
      return null;
    }
  }
  // ========================================================
  ...
  _snapshotCandidates() {
    try {
      ...
      for (let r = 0; r < this.board.cells.length; r++) {
        const row = this.board.cells[r];
        for (let c = 0; c < row.length; c++) {
          const cell = row[c];
          if (!cell) continue;
          snap.push({ r: r, c: c, cands: cell.candidates ? Array.from(cell.candidates) : [] });
        }
      }
      return snap;
    } catch (e) { return null; }
  ...
  /**
   * Q5：恢复棋盘候选数（撤销 updateCandidates 的副作用）
   * @private
   */
  _restoreCandidates(snap) {
    try {
      if (!snap || !this.board || !this.board.cells) return;
      for (let i = 0; i < snap.length; i++) {
        const s = snap[i];
        const cell = this.board.cells[s.r] && this.board.cells[s.r][s.c];
        if (!cell || !cell.candidates) continue;
        cell.candidates.clear();
        for (let k = 0; k < s.cands.length; k++) cell.candidates.add(s.cands[k]);
      }
    } catch (e) {}
  }
  ...
  _findNextDeduction() {
    ...
    // 创建 TechRater 实例（从全局读取，兼容浏览器 window 与 Node globalThis）
    // 2026-08-03：裸标识符 TechRater 在 ESM 严格模式下未声明会抛 ReferenceError，
    // 统一改从 globalThis 读取
    const TR = (typeof globalThis !== 'undefined' && globalThis.TechRater) || null;
    if (!TR) {
      console.warn('HintSystem: 全局 TechRater 未加载（需 <script src="core/tech-rater.js">）');
      return null;
    }

    // ===== 2026-08-03：教学目标技巧优先（直到首次展示） =====
    // 每次 getHint 都尝试目标技巧；如果当前盘面可检测到，立即展示
    // 支持"延迟结构"关卡（如 404 三子法、704 X-Wing：需先填数后结构浮现）
    if (!this._targetTechShown && this.preferredTechnique) {
      try {
        // V4.3.15 必要性验证：先证明目标技巧"真必要"再展示。
        // 实测发现：全技巧求解时最低技巧优先，xWing 步骤从不出现
        // （裸单/数对/区块排除先解出，xWing 伪必要）；且「基础四技+目标技巧」
        // 白名单走不通（中间技巧也必需）。正确判定：
        //   无目标技巧的白名单解不出 且 全技巧可解 → 目标技巧真必要 → 展示其步骤
        const allTechs = (TR.TECH_PRIORITY_9) || ['nakedSingle', 'cageUnique', 'hiddenSingle', 'rule45', 'nakedPair', 'hiddenPair', 'pointingClaiming', 'nakedTriplet', 'xWing', 'swordfish'];
        const withoutTarget = allTechs.filter(t => t !== this.preferredTechnique);
        const simWithout = new TR(this.board);
        simWithout.techPriority = withoutTarget;
        const rWithout = simWithout.solve(500);
        if (rWithout && !rWithout.solvable) {
          // 目标技巧必要：全技巧求解，目标技巧必然出现在 steps 中
          const simFull = new TR(this.board);
          simFull.solve(500);
          const simSteps = simFull.getSteps() || [];
          const targetStep = simSteps.find(s => s.technique === this.preferredTechnique);
          if (targetStep) {
            this._targetTechShown = true;
            // v2.0：45法则（星衡）首次展示 → 通知游戏层解锁"45账本"提示
            if (this.preferredTechnique === 'rule45') {
              try {
                if (typeof window !== 'undefined') {
                  window.dispatchEvent(new CustomEvent('rule45Taught', { detail: { technique: 'rule45' } }));
                }
              } catch (eE) {}
            }
            return this._convertTechRaterResult(targetStep);
          }
        }
        // 目标技巧非必要（无它也能解）或不可用：降级到正常流程
      } catch (e) {
        // 降级到正常流程
      }
    }
  ...
  _convertTechRaterResult(step) {
    ...
    let targetCells = this._buildTargetCells(step, evidence, technique);
    // 2026-08-03：elimination 类型（hiddenPair 等排除无裸单时）——
    // 目标格指向结构格子而非填数格，Level 3 提示为"排除"而非"填数"
    if (stepType === 'elimination' && evidence) {
      if (Array.isArray(evidence.pairCells) && evidence.pairCells.length > 0) {
        targetCells = evidence.pairCells.map(([r, c]) => ({
          row: r,
          col: c,
    ...
    // 归一化 eliminatedCells -> eliminatedPositions（供 hint-adapter 红叉动画）
    // 2026-08-03：tech-rater 证据链补全后，xWing/swordfish/pointingClaiming/nakedTriplet
    // 均携带 eliminatedCells=[{row,col,num}]，统一在此转换为渲染层字段
    let normEvidence = evidence;
    if (evidence && typeof evidence === 'object') {
      let ep = Array.isArray(evidence.eliminatedPositions) ? evidence.eliminatedPositions.slice() : [];
      if (ep.length === 0 && Array.isArray(evidence.eliminatedCells)) {
        ep = evidence.eliminatedCells
          .filter(p => p && typeof p === 'object' && typeof p.row === 'number' && typeof p.col === 'number')
          .map(p => ({ row: p.row, col: p.col, value: (p.num !== undefined ? p.num : p.value) }));
      }
      normEvidence = Object.assign({}, evidence, { eliminatedPositions: ep });
    }
    ...
    return {
      ...
      eliminationSteps: normEvidence && normEvidence.eliminatedPositions ? normEvidence.eliminatedPositions.length : 0,
      // 向后兼容字段
  ...
  _buildExplanation(step, evidence, technique) {
    ...
    if (technique === 'rule45') {
      const scopeLabel = this._scopeLabel(evidence.scopeType, evidence.scopeIndex);
      const type = (evidence.subtype === 'innie' || evidence.subType === 'innie') ? '内突' : '外突';
  ...
  _generateDialogue(charId, deduction, level) {
    ...
    if (level === 3) {
      // V4.3.26：Level 3 优先用角色模板 + 证据链填充生成精准文案（含具体数字/笼和），
      // 无可用证据时回退旧的三级渐进文案
      const tplMsg = this._tryTemplateDialogue(charId, deduction);
      if (tplMsg) return tplMsg;
  ...
  /**
   * V4.3.26：尝试用本地角色模板 + 证据链渲染精准提示
   * 有证据字段（cageSum/filledNums/pairValues/num）且渲染后无 '?' 时返回文案，否则 null（回退旧逻辑）
   * @private
   */
  _tryTemplateDialogue(charId, deduction) {
    try {
      const technique = deduction && deduction.technique;
      if (!technique) return null;
      const ev = (deduction && deduction.evidence) || {};
      const ctx = { num: deduction.num, arc: this.arc };
      const msg = renderHint(charId, technique, ev, ctx);
      if (!msg) return null;
      // 占位符未全部替换（残留 {xx}）或出现 '?' 缺失标记 → 回退旧文案，避免尴尬
      if (/\{[a-zA-Z]+\}/.test(msg) || msg.indexOf('?') >= 0) return null;
      return msg;
    } catch (e) {
      return null;
    }
  ...
  resetProgress() {
    try {
      ...
      console.warn('HintSystem.resetProgress 异常:', e);
    }
  }
  ...
  getHintProgress(deduction) {
    try {
      ...
      console.warn('HintSystem.getHintProgress 异常:', e);
      return 0;
    }
  }
