 ...
 *  触发条件（三重锁定）：
 *  1. 长时沉顿：最后有效操作 ≥ 60 秒
 *  2. 高崩塌势能：落子后新增裸单数 ≥ 4 格
 *  3. 无提示盲解：落子前 180 秒内无提示/角色触发
 *
 *  【补丁1】判定时机扩展：
 *  - 填数（fill）时触发
 *  - 删除笔记导致候选数收敛为1 + 连锁坍塌 ≥4格 时触发
 *
 *  使用方式：
...
class EurekaDetector {
  ...
  /**
   * 删除笔记时检测Eureka
   * 当删除笔记导致某格候选数收敛为1，且引发连锁坍塌时触发
   *
   * @param {number} row - 行号
   * @param {number} col - 列号
   * @param {number} value - 删除的候选数
  ...
  static _detectTechnique(row, col, value, board) {
    ...
    if (candidates === 1) {
      return 'nakedSingle'; // 裸单
    }

    // 检查是否是笼子唯一组合
    const cageIds = this._getCellCageIds(row, col, board);
    if (cageIds.length > 0) {
      return 'rule45'; // 简化为45法则
    }

    return 'hiddenSingle'; // 隐单
  }
