 ...
 *
 *  注意：自动候选数产生的笔记（source: 'auto'）不计入推理分析。
 *
 *  使用方式：
 ...
 * @property {number} value - 候选数值（1-9）
 ...
 * @property {boolean} autoCandidatesOn - 是否开启了自动候选数功能
...
class ReasoningMonitor {
   ...
   * @param {boolean} autoCandidatesOn - 是否开启自动候选数
   */
  ...
  static _calcAccuracyWithSolution(solution) {
    ...
    for (const note of manualNotes) {
      ...
      if (!cellCandidates[key]) {
        cellCandidates[key] = new Set();
      }
      ...
      if (note.action === 'add') {
        ...
        if (cellCandidates[key].has(note.value)) {
          cellCandidates[key].delete(note.value);
        } else {
          cellCandidates[key].add(note.value);
        }
    ...
    for (const key in cellCandidates) {
      ...
      if (!correctNum) continue; // 跳过没答案的格子

      for (const cand of cellCandidates[key]) {
        totalCandidates++;
        // 正确答案的候选数算"正确"吗？
        // 答案是唯一的，所以只有等于正确答案的候选数才是"最终正确"的
        // 但从推理角度，只要不排除正确答案就算"对的"
        // 这里用"候选数包含正确答案"作为准确率指标
        if (cand === correctNum) {
          correctCandidates++;
        }
      }
    ...
    if (totalCandidates === 0) return 0.5;

    // 修正：准确率应该是"包含正确答案的格子数 / 有笔记的格子数"
   ...
   */
  static getReasoningState(solution = null) {
    // Eureka 状态在 5 秒后自动衰减
  ...
  static debugPrint() {
    ...
    console.log(`  自动候选数: ${state.autoCandidatesOn ? '开启' : '关闭'}`);
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { ReasoningMonitor };
}
