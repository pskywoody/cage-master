...
# 角色音色配置
VOICES = {
    'VO_CK': {'voice': 'zh-CN-YunxiNeural', 'rate': '-5%', 'pitch': '-2Hz'},    # 守笼人 - 温和男声
    'VO_R':  {'voice': 'zh-CN-YunjianNeural', 'rate': '+5%', 'pitch': '+2Hz'},    # 阿岩 - 青年男声
    'VO_P':  {'voice': 'zh-CN-YunxiNeural', 'rate': '-15%', 'pitch': '-5Hz'},     # 设局人 - 沉稳男声（降速降调）
    'VO_W':  {'voice': 'zh-CN-YunxiNeural', 'rate': '+10%', 'pitch': '+2Hz'},     # 星辰梭 - 高傲男声（快语速）
    'VO_RE': {'voice': 'zh-CN-YunxiNeural', 'rate': '-15%', 'pitch': '-3Hz'},     # 残局守护者 - 机械感
    'VO_S':  {'voice': 'zh-CN-XiaoxiaoNeural', 'rate': '+0%', 'pitch': '+0Hz'},   # 系统
}
...
async def main():
    print('=' * 50)
    print('  Edge TTS Voice Generator')
    print(f'  Total: {len(DIALOGUES)} lines')
    print('=' * 50)
    
    tasks = []
    for vo_id, prefix, text in DIALOGUES:
        tasks.append(generate_voice(vo_id, prefix, text))
